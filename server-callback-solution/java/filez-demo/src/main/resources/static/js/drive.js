 ...
 * @param isInFrame Whether to open in current tab
 * @param callback Whether to open in current tab
 */
function openDoc(docId, action, isInFrame, callback) {
    if (!docId) return;
    let url = `/v2/context/openDoc?docId=${docId}&action=${action}`;
    url = isInFrame ? `${url}&isInFrame=true` : url;
    // Get standard integration URL from backend
    fetch(url).then(res => {

        if (!res.ok) {
            showToastInParent(`Open failed: ${res.statusText}`);
            return;
        }
        res.text().then(res => {
...
/**
 * Publish message to open file in current page
 */
