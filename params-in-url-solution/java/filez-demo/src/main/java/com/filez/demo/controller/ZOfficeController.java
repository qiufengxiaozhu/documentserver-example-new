...
public class ZOfficeController {
    ...
    @Log("Build frontend integration URL")
    @ApiOperation(value = "Build frontend integration URL for requesting ZOffice service")
    ...
    public ResponseEntity<String> getDriverCbUrl(@RequestParam(name = "docId") String docId,
        ...
        log.info("Frontend integration base URL: {}", builder);

        // Note: Please ensure the parameter order is consistent, otherwise it will cause errors
        builder.addParameter("repoId", demoConfig.getRepoId());
        builder.addParameter("action", action);
        builder.addParameter("docId", docId);

        // 1. Transfer user information
        ...
        // 3. When the business system already has upload/download addresses, there's no need to define new interfaces, just provide the upload/download addresses in the URL in advance, as shown below:
        // TODO: Note that if the download address IP is different from the document platform service deployment IP, the download address IP needs to be added to the whitelist
        String downloadOrUploadUrl = ServletUriComponentsBuilder.fromCurrentContextPath()
                ...
                .toUriString()
                .replace("localhost", demoConfig.getHost()); // Replace with the IP of this project
        // 4. File upload and download addresses, used to return the latest document content to the business system after editing in the document platform
        builder.addParameter("downloadUrl", downloadOrUploadUrl);
        builder.addParameter("uploadUrl", downloadOrUploadUrl);

        // 5. Request body authentication information, additional parameters can be added
        ...
        // 6. Current timestamp, ZOffice will verify timeliness
        ...
        log.info("Frontend integration complete URL: {}", builder);
    ...
    public ResponseEntity<String> compareDoc(@ApiParam(value = "Document A ID") String docAid,
        ...
        builder.setPath(zOfficeConfig.getContext());
        log.info("Document comparison base URL: {}", builder);
        ...
        // 3. Download URL
        String downloadOrUploadUrl = ServletUriComponentsBuilder.fromCurrentContextPath()
                .path(demoConfig.getContext().substring(1) + "/{docAid}/content")
                .buildAndExpand(docAid)
                .toUriString()
		        .replace("localhost", demoConfig.getHost()); // Replace with the IP of this project
        builder.addParameter("downloadUrl", downloadOrUploadUrl);

        // 4. Download URL B
        String downloadOrUploadUrlB = ServletUriComponentsBuilder.fromCurrentContextPath()
                .path(demoConfig.getContext().substring(1) + "/{docBid}/content")
                .buildAndExpand(docBid)
                .toUriString()
		        .replace("localhost", demoConfig.getHost()); // Replace with the IP of this project
        builder.addParameter("downloadUrlB", downloadOrUploadUrlB);
